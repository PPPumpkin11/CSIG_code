...
class Video_processor(Image_processor):
    ...
    def toframe(video_file_path):
        ...
        # 如果是指定类型，就完成视频与帧之间的转换
        frame_list = video2frame(video_file_path, frame_save_dir=video_basename+'_frames')
        return video_basename, frame_list

    @torch.no_grad()
    def process_video(self, video_file_path, video_output_dir):
        self.output_dir = video_output_dir
        if os.path.isdir(video_file_path):  # 判断路径是否是一个文件夹的路径，而不是一个文件的路径，文件夹进行帧的转换与文件所用方法不同
        ...
        elif os.path.exists(video_file_path):  # 如果不是文件夹，那就是文件，先判断这个文件是否存在，如果存在的话，那它会是一个视频，用上面的函数对视频进行帧的转换
            video_basename, frame_list = self.toframe(video_file_path)
        else:
            raise('{} not exists!'.format(video_file_path))
        # 显示将视频分为多少帧，名称是什么，保存到哪一个文件夹中
        ...
        internet_loader = self._create_single_data_loader(dataset='internet', train_flag=False, shuffle=False, file_list=frame_list)  # 类似于pytorch中的Dataloader函数
        counter.start()
        ...
        for test_iter, meta_data in enumerate(internet_loader):
            ...
            ##------------
            video_basename ,VideoExt = os.path.splitext(video_file_path)
            PicIdx = format(test_iter, '06d')
            pic_path = video_basename + '_frames/' + PicIdx + '.jpg'
            ActionName = video_basename[20:] # 此处选手可根据自己的视频数据修改
            SmplParams_name = './demo/CSIG/Action/' + ActionName + '.npy'
            J3d_smpl = np.reshape(results[pic_path][0]['j3d_smpl'],(1,72))
            SmplPose = np.reshape(results[pic_path][0]['poses'],(1,72))

            if test_iter == 0:
                SmplParams = SmplPose
            else:
                SmplParams = np.concatenate((SmplParams,SmplPose),axis = 0)
            ...
            PicIdx = format(test_iter, '06d')
            pic_path = video_basename_ + '_frames/' + PicIdx + '.jpg'
            # SmplParams_name = './demo/CSIG/Action/' + ActionName + '.npy'
            # J3d_smpl = np.reshape(results[pic_path][0]['j3d_smpl'], (1, 72))
            SmplPose = np.reshape(results[pic_path][0]['poses'], (1, 72))
        ...
        np.save(SmplParams_name, SmplParams)
